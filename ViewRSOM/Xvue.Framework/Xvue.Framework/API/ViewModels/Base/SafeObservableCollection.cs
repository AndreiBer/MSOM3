...
namespace Xvue.Framework.API.ViewModels.Base
{
    ...
    public class SafeObservableCollection<T> : IList<T>
    {
        /// <summary>
        ...
        public ObservableCollection<T> ObservableCollection
        {
            get
            {
                ...
                {
                    throw new InvalidOperationException("Observable Collection only accessible from UI thread");
                }
        ...
        public bool Remove(Predicate<T> match)//amma++++
        {
            bool result = false;
            T item = this._list.Find(match);
            if (item != null)
            {
        ...
        public T IsItemPresent(Predicate<T> match)//amma++++
        {
        ...
        //xvue addition
        public void AddThreadSafeItem(T newItem)
        {
            using (TimedLock timedLock = AcquireLock())
            {
                this.Add(newItem);
            }
        }
